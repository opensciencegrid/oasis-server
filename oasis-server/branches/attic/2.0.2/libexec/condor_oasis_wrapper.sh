...
f_source_setup(){
    ...
    # It will define a few relevant environment variables, i.e.
    #       - VO
    #       - OASIS_CONTENT_DISTRIBUTION_TOOL
    #       - OASIS_VO_SRC_DIRECTORY
    #       - OASIS_VO_DEST_DIRECTORY
    #
...
f_reverse_sync(){
    ...
    # the destination area, i.e. /cvmfs/
    #

    f_print_info_msg "Reverting last updated."
    rsync -a -l --delete $OASIS_VO_DEST_DIRECTORY/ $OASIS_VO_SRC_DIRECTORY/
    rc=$?
    if [ "$?" == "0" ]; then
...
f_main(){
    ...
    # run python code
    /usr/bin/oasis --oasiscmd=runpayload $@ 
    /usr/bin/soasis  # temporary name
