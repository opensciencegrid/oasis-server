...
class TaskManager(object):
    ...
    def start(self):
        '''
        starts all object Task
        Tasks are object of a Thread class 
        '''

        self.log.debug("start: Starting")
        for taskname in self.oasis.tasksconf.sections():
            if self.oasis.tasksconf.getboolean(taskname, 'enabled'):
                self.log.info('start: Adding taskname %s' %taskname)
